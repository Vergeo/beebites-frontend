import 'package:flutter/material.dart';

class BeeStyle {
  static const Color lightWhite = Color.fromARGB(255, 255, 255, 255);
  static const Color white = Color.fromARGB(255, 242, 242, 242);
  static const Color darkWhite = Color.fromARGB(255, 230, 230, 230);

  static const Color darkBlack = Color.fromARGB(255, 0, 0, 0);
  static const Color black = Color.fromARGB(255, 13, 13, 13);
  static const Color lightBlack = Color.fromARGB(255, 26, 26, 26);

  static const Color darkOrange = Color.fromARGB(255, 254, 188, 118);
  static const Color orange = Color.fromARGB(255, 254, 176, 93);
  static const Color lightOrange = Color.fromARGB(255, 254, 164, 67);

  static const Color darkBlue = Color.fromARGB(255, 110, 138, 211);
  static const Color blue = Color.fromARGB(255, 90, 112, 205);
  static const Color lightBlue = Color.fromARGB(255, 71, 107, 199);
}
